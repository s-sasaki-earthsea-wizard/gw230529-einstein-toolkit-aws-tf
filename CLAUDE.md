...
- **中断帯を重ねた (2026-09-19)**。ψ4 ピークの縦破線は撤去し、代わりに
  **12 回の spot 中断を simulation time に落として帯で描く**。
  帯 = [次ノードが復帰した checkpoint (点線), 失われたノードの到達点 (破線)]
  で、**二重計算された区間**そのもの。合計 168 M⊙ = 全体の 9.6%
...
- **分布そのものが語る**: 5 回が t<92 M に固まり (8/27 の c7a 逼迫)、
  ...
  **下段の残差は 12 回のどこにも段差を作らない**。recovery が計算を
  ...
  通常の乖離だけ
- 帯は alpha 0.13 で重ね描きし、**重なったぶんだけ濃くなるのを利用する**
  (t=55〜58 M は 6 回計算された)。凡例のスウォッチだけ alpha 0.45 —
  データを潰さない濃さだと凡例では見えないため
