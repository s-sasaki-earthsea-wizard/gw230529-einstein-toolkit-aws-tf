...
**【2026-09-16 訂正】140 GB の出所は論文だった** — `docs/2603.07374v2.pdf`
Sec. 2 に「2 nodes / 256 cores / 30 hours / ~7680 CPU-hours / ~140 GB」と
書いてある。当時「出所不明なのでもう使わない」としたのは、論文本文を
読んでいなかったから。下記「論文 (arXiv:2603.07374) と parfile の対応」。
**この表の「実測」側は公開 tarball のログなので有効**で、
訂正されるのは「出所が分からない」という記述の方。
...
**ノード稼働帯の図は台帳から描く** (`make ledger-chart`、2026-09-16 追加):

- `run_ledger.sh` に `--tsv` を足して機械可読出力にし、`plot_ledger.py` が
  それを読む。素材が `output/` ではなく `logs/` なので、**`make pack-results`
  で results/ を畳んだ後も再生成できる**。梗概用に静的図が要る (#27 と同根で、
  ダッシュボードは private artifact なのでセッション外に残らない)
- **色は「中断されたか」ではなく「仕事が残ったか」**。次に起動したノードが
  自分より後の iteration から再開していれば checkpoint を残せたと判定する。
  S3 の slot は 2 世代しか持たないので、**台帳がこの事実の唯一の記録**。
  14 台中 6 台が何も残せずに死んだ (合計 2h02m)
- **【2026-09-17 訂正】この 2h02m は「捨てた課金時間」ではない。** 6 台とも
  `spot-interruption` かつ稼働 1 時間未満で、**EC2 が中断した spot は最初の
  1 時間以内なら課金されない**。捨てたのは wall clock だけ。
  スクリプトはこれを**主張せず判定する** — reason と稼働秒を見て、全部が
  条件を満たすときだけ「課金ゼロ」と出力する (1 時間を超えた同じ状態の
  ノードは課金されるので、条件つきでしか言えない)
- **図が数えるのは run であって bucket ではない** (2026-09-17)。完走 2 分後に
  手動起動された 15 台目は台帳にはあるが run には属さない。含めると wall clock が
  17 分伸びて **45h50m vs 45h33m** の差になる。梗概は 45h33m (D7) を採ったので、
  図は served 14 台で span / duty / 実効を出し、`verify()` が
  「run の合計 + 除外分 = 台帳の合計」という恒等式で両者を突き合わせる
- **図中に幾何単位を出さない** (梗概 D8)。Cactus の時間は $M_\odot$ だが
  講演の聴衆は CFD なので、到達時刻は **8.62 ms** と書く
  (1 $M_\odot$ = 4.9254909 μs)。`pdf.fonttype = 42` も併せて設定 —
  既定の Type 3 は拡大時に崩れる
- **インスタンスタイプはログのどこにも無い**。NUMA 修正が restore 後に吐く
  `NNN GiB available` が唯一の手がかりで、384 GiB と 768 GiB は完全に分離する
  (365/366 vs 734)。**user_data が IMDS から instance-type を記録していれば
  要らない推定** — 中断されたノードは EC2 が 1 時間で忘れるので後から引けない。
  BNS repo で直す候補
- **描いた帯の幅を台帳の稼働時間と突き合わせてから保存する**。8/29 に帯が
  2px に潰れた事故 (無防備な `.replace()` が日付跨ぎを取りこぼした) の再発防止。
  合計 3 本 (span / uptime / evolution) も `#totals` と照合する
- 中断の時間帯集中を読ませるため、背景に **US Pacific の業務時間
  (Mon–Fri 08–17)** を敷く。閾値を自分で選ばないための既定

**データ側の確定事項** (2026-08-29 実測):

- **ψ4 は mp_psi4.h5 を使うこと**。ASCII の multipole 出力は同じ 15.36 M
  サンプリングしか持たず、参照 run の密な .asc と混同しやすい (一度誤読した)。
  **生の h5 は 127 行だが、重複除去後は 114 点** (dt=15.36 M、t≤1735.68)。
  差の 13 行は 12 回の復帰 (recovery) が再発行したもので、common.py の
  dedup_sorted が落としている — **重複除去が実際に効いている証拠がこの差**。
  合体まで 7〜17 サンプル/周期でチャープ図は成立、リングダウン末尾は角張る。
  ピークは **t=1213.44 M** で既知値と一致
...
**図は幾何単位と SI の 2 セット作る (2026-09-17 追加)**:

- `UNITS ?= geom si` (post.mk)。SI 版は `_si` サフィックス付きで別ファイルに
  出るので衝突しない。`make movie` だけ 2 周目がタダではないので、
  片方でいいときは `UNITS=si` で半分にする
- **梗概 D8 と同じ理由** — Cactus の幾何単位はギャラリーの参照 run と
  比較するのに要るが、OpenCAE の聴衆には読めない。
  1750 M = 8.62 ms、500 M = 738 km、ρ=1.3e-3 → **7.8e17 kg/m³**
  (核密度 2.8e17 kg/m³ と並べられる数字になるのが一番の収穫)
- **密度は kg/m³ (2026-09-25 に g/cm³ から変更、Syota の指示)**。
  コンパクト天体の論文は g/cm³ で書くのが慣習だが、それは「SI」と名乗るセットの
  中で唯一の CGS 単位だった — 他の SI の資料と行き来する読者は、その 1 軸だけ
  10³ ずれていることを知っていなければならない。換算係数は
  `M_SUN_KG_PER_M3 = c⁶/(G (GM_sun)²) = 6.1758e20` で、g/cm³ 版にあった
  `× 1e-3` が消えただけ。rho_max_si / rho_panel_si / rho_xy_si がまとめて追従する
- **換算係数は common.py に集約**し、IAU nominal GM_sun から導出する。
  kuibit の `unitconv` は GM_sun を G で割って質量にしてから掛け直すので
  精度を落とす (相対差 2.5e-5、図には出ないが定数の出所は揃えた)。
  `plot_ledger.py` が持っていた `M_SUN_MICROSECONDS` もこれに統合した —
  **同じ物理定数が 1 パッケージに 2 つあるのは、刷り上がったスライドで
  食い違いが見つかる形**
- **Ψ4 だけは SI にしても意味が増えない**。SI 次元は s⁻² (無次元歪み h の
  2 階時間微分) なのでそう書いてあるが、非専門家が知っているのは h の方。
  h を出すには 2 階時間積分が要るので**意図的にやっていない**。
  欲しくなったら FFI を足す話になる
- 質量は M⊙ ではなく 10³⁰ kg で出す。質量だけは非 SI でも読める単位だが、
  他の軸が SI の図に混ぜると「この図は天文の単位だ」と読ませてしまう

**参照 run との重ね図 `psi4_vs_reference{,_si}` (2026-09-18 追加)**:

- 上段: **参照は線 (橙 lw 2.0)、本 run は白抜き丸 (青 ms 3.4) を全サンプルに**。
  「公開値の線の上に自分の点が乗る」という最も手垢のついた一致の示し方。
  **2 本とも線にする案は全滅 (2026-09-18 に実物で確認)** — 同じ太さだと
  1 本に融合し、薄い太線 + 濃い細線だと参照が後光になり、**後光は誤差帯に
  見える** (Syota の指摘: 「区間推定みたいになっちゃう」)。太さの軸では
  解けない問題なので、マーカーという離散的な物体に逃がした。
  おまけに 15.36 M のサンプリングが可視化される — 下段が測っている刻みそのもの
- **markevery=2 も却下**。少し綺麗になるが「保存された全サンプルが線に乗る」
  という強い主張を捨てることになり、実際に合体付近で極値を飛ばす
- **破線も却下** — この図は補助線 (peak) に破線を使っていて規約が衝突するし、
  114 点の折れ線に破線を乗せると角で切れて欠測に見える (使い捨て比較で確認)。
  「縦に 2 枚並べる」案も却下 — 目は別パネルを 1% より細かく比べられない
- 下段: 複素差 |Δψ4| / max|ψ4| の log 軸 (実部の差だと半周期ごとに符号が暴れる)。
  目盛は天井から 4 桁刻みで置く — 14 桁を短いパネルに入れると天井に目盛が
  無くなり、最大値がどの桁の下にいるか読めなくなる
- **実測: t=0 で 5×10⁻¹⁷ (初期データ同一)、静穏期 ~10⁻¹⁴、inspiral〜合体で
  13 桁育って最大 4.4×10⁻⁴ (t=1382 M)、以後横這い**。480 rank/12 ノードと
  192 rank/1 ノードは同じ計算ではなく、合体が食い違いを増幅する — それでも
  10⁻³ を切る、が結果。CFD の聴衆には初期値鋭敏性の話として通じる
- 参照の `mp_psi4_l2_m2_r500.00.asc` は **同じ 15.36 M 刻みで時刻が完全一致**、
  補間ゼロ (刻みが違う参照が来たら本 run の時刻へ補間して警告を出す)。
  `fetch_inputs.sh --reference` が .out に加えてこの .asc も展開し、post.mk が
  `$(INPUTS_DIR)/bhns_20252103` を `/ref` に read-only mount する (無ければ mount
  も図もスキップ)
- 出典はギャラリー配布の `bhns_20252103.tar.gz` (ETK の download host)。
  Zenodo ではないので、キャプションに書くときは注意
- **中断を縦線で重ねた (2026-09-19)**。ψ4 ピークの縦破線は撤去し、代わりに
  **12 回の復帰を simulation time に落とす**。破線 = ノードが失われた
  到達点、点線 = 次のノードが復帰した checkpoint。間が**二重計算された区間**で
  合計 168 M⊙ = 全体の 9.6%
- **【2026-09-25 訂正】中断は 13 回、復帰は 12 回 (Syota の指摘)**。
  それまで本 repo・メモリ・講演 repo が「中断 12 回」と書いていたが、
  **ログの逆走が数えるのは復帰であって中断ではない**。13 回目の中断は
  i-0862 (8/27 12:18〜12:29 UTC、稼働 10 分) — checkpoint restore の途中で
  回収され、Cactus 起動前に死んだのでログにも simulation time 軸にも痕跡が無い。
  その損失は次の復帰に畳み込まれ、**最初の破線/点線ペア (92 → 55 M) は
  中断 2 回分をまたいでいる**。
  凡例は `12 resumptions (13 interruptions)`。13 は台帳にしか無いので、
  `ledger.tsv` があれば読み、**中断されて evolve したノード数 = ログの復帰数**
  を確かめてから載せる (一致しなければ括弧を出さない)。台帳が無ければ復帰数だけ。
  使い分け: **「12 回の recovery で NUMA が再発しなかった」は復帰の話なので正しい**。
  「中断 12 回」「12 回すべて無人処理」は中断の話なので 13 が正しい
- **半透明の帯は最初に入れたが撤去した (Syota の判断)**。重なりが濃くなって
  「何度潰されたか」が出る利点はあったが、**x 軸の 1/4 にデータの下敷きが
  敷かれる**代償の方が大きい。この図の主役は波形であって reclaim ではない
- **中断は台帳ではなく Cactus ログから取る**。`ledger.tsv` は S3 から再構築
  するので図が AWS セッションに依存してしまう。ログは図が既に要求している
  run データと一緒に来るし、**iteration カウンタが逆走する = recovery 1 回**
  で一意。`make ledger-chart` の evolve したノード 12 台と一致を確認済み
  (最後の 1 件だけ 23100 vs 23096 — info 行が 4 iteration 刻みなだけ)
- **分布そのものが語る**: 5 回が t<92 M に固まり (8/27 の c7a 逼迫)、
  4 回が t=1270〜1386 M — リングダウンのど真ん中。にもかかわらず
  **下段の残差は 12 回の復帰のどこにも段差を作らない**。recovery が計算を
  元の場所に戻した証拠で、育っているのはランク分割の違いによる
  通常の乖離だけ
- **凡例は 3 行に折る**。1 行だと合体を越えて伸び、注釈すべき波形の上に
  乗ってしまう (Syota の指摘)。凡例の見本は破線 (Line2D) 1 本
- アスペクトは **2.1 (12.6 × 6.0 inch)**。x 軸にチャープ・中断・13 桁の残差の
  3 つが乗るので横長が要る。スライドにもそのまま載る
...
### 論文 (arXiv:2603.07374) と parfile の対応 (2026-09-16 確認)

**「gallery の MR 設定を再現した」は格子については正しく、run 長については
正しくない。** `docs/2603.07374v2.pdf` と上流 parfile を初めて突き合わせた。
これまで一致の根拠は合体時刻と波形 (出力側) だけで、**設定側からは
確認していなかった**。

**コード単位は M☉。総質量 5 M☉ ではない。** 1 M☉ = 1.4766 km = 4.9255 μs。
本ファイルの「t=1750 M」「dx=19.2 M」「初期分離 30 M」は全て M☉ の意味で
一貫している。**総質量と読むと全部 5 倍ずれる**ので、外に出す文書では
明示すること。

| 量 | parfile | 換算 | 論文 |
| --- | --- | --- | --- |
| finest grid spacing | 19.2 / 2⁷ = 0.15 | **221.5 m** | **222 m** |
| domain (全幅) | 1344 (±672) | 1984.6 km | ~1985 km |
| finest level 半径 | `radius_N[7]` = 10 | 14.77 km | ~15 km |
| **GW 抽出半径** | `Multipole::radius[4]` = 500 | **738.3 km** | **~738 km** |
| 初期分離 | 30.0 | 44.30 km | 45 km |
| refinement levels | `num_levels` = 8 (level 0 + 7) | — | "seven refinement levels" |

- **738.3 km がドンピシャ**。丸めの余地が無く、偶然では出ない
- HR / LR も逆算で辻褄が合う: 162 m → dx=14.0 (96 セル)、310 m → dx=26.88
  (50 セル)。**いずれも 1344/dx が整数**で、論文の「解像度を変えるときは
  格子間隔だけ変えて refinement 数は増やさない」と整合する
- 論文自身が **"The public gallery example corresponds to the MR setup."**
  と明記している。本 repo が使うのは gallery parfile そのもの (SHA-256 pin) なので、
  **出力側の一致とは独立に、設定側からも MR であることが確認できた**

**【要注意】進化時間は論文の run より短い。**

論文は *"All simulations are evolved for at least ~10 ms after merger."*

| | t | post-merger |
| --- | --- | --- |
| 本 repo の本番 run | 1750 M☉ = 8.62 ms | **5.11 ms** |
| gallery parfile の既定 | 2000 M☉ = 9.85 ms | **6.34 ms** |
| 参照 tarball | 3041 M☉ = 14.98 ms | 11.47 ms |

- **公開 parfile の 2000 M ですら論文基準を満たさない。** 論文の図は
  t=3041 M まで延長した run で作られている (「parfile の 2000 M を超えて
  延長実行されている」の理由がこれ)
- 「再現した」と外に書くときは **格子設定の再現であって run 長の再現ではない**
  と添えること。本 repo の目的 (定性的な波形・合体の再現) には影響しないが、
  黙っていると過大主張になる
- 合体時刻の定義も論文とは別: 論文は strain h の (2,2) モードのピーク、
  本 repo の t≈713 M は ψ4 ピーク − 抽出半径。数 M のずれが出る

**論文の記述と公開 tarball が食い違っている** (上記「インスタンス選定の再評価」):

| | 論文 Sec. 2 | 公開 tarball の実測 |
| --- | --- | --- |
| ノード / コア | 2 nodes / 256 cores | 12 ノード / 480 rank (40 コア/ノード) |
| 時間 | 約 30 時間 | 46.4 h (t=3041 M) |
| メモリ | 約 140 GB | 438.5 GB |

- 論文側は内部整合している — COSMA8 は 128 コア/ノードなので 2 ノード = 256 コア。
  一方 **40 コア/ノードは COSMA8 の構成ではない** (COSMA7 は 28 コア/ノード)
- したがって「論文が誤り」ではなく、**論文の記述と公開されている出力が
  別の run を指している**と読むのが妥当。断定はできない
- **格子設定は一致しているので MR 再現性には影響しない。** 食い違うのは
  運用側の数字だけ。本 repo の判断 (単一ノード c7a.48xlarge) は
  公開 tarball 側の実測に基づいており、そこは変わらない
...
### 積んだ PR の始末 (2026-09-18 の事故から)

**base の PR が merge 済みのブランチに PR を merge しても main には届かない。**
#29 と #32 は `feat/postprocessing` (= #28、8/29 に main へ merge 済み) の上に
積まれ、上から順に merge されて 13 コミットがそこに取り残された — 3 週間、
main に `make ledger-chart` も `UNITS` も FIS も無かった。回収は #33。

- 積む場合は **base の PR が open のうちに下から merge** する
- base が先に main へ入ったら、上の PR は **main に付け替えてから** merge する
- 見分け方: `git log origin/main..origin/<branch>` が空でない merge 済み
  ブランチは、取り残しの疑い
